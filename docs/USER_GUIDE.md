...
| `src/codex/` | Codex-specific config and exec-policy sources |
...
### Codex layer (Compiled)

> The `.codex/` and `.agents/` directories are generated by the compiler. Edit source files in `src/` instead.

| File | Type | Purpose |
|------|------|---------|
| `.codex/config.toml` | Config | Project-scoped Codex defaults for sandbox, approvals, model, and web search |
| `.codex/agents/*.toml` | Subagents | Codex subagents compiled from `src/agents/*.md` |
| `.codex/rules/*.rules` | Exec policy | Optional Codex command approval rules compiled from `src/codex/rules/` |
| `.agents/skills/*/SKILL.md` | Skills | Codex skills compiled from `src/skills/*` |

Codex uses `AGENTS.md` directly for shared instructions. The behavioral repo rules in `src/rules/` are not auto-translated into Codex `.rules` files.
