...
| `.cursor/skills/memory-templates/SKILL.md` | Skill | 記憶檔不存在時的建檔模板（四類 Core Logic + Framework Detection → `project-framework.mdc`） |
...
| `.cursor/skills/sdd-bootstrap/SKILL.md` | Skill | 空白／骨架專案進入 SDD 需求蒐集，完成後可交棒 `sdd-designer` |
...
| `.cursor/agents/doc-fetcher.md` | Subagent | 擷取外部文件／API 說明，供實作前後對照 |
| `.cursor/agents/sdd-designer.md` | Subagent | SDD 需求完成後的規格與設計產出（與 `sdd-bootstrap` 銜接） |
