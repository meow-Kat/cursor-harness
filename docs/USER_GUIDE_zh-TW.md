...
| `src/codex/` | Codex 專用設定與 exec-policy 規則來源 |
...
> `.codex/` 與 `.agents/` 目錄由編譯器生成。請編輯 `src/` 中的源碼檔案。

| 檔案 | 類型 | 用途 |
|------|------|------|
| `.codex/config.toml` | Config | Codex 專案層預設設定：sandbox、approval、model、web search |
| `.codex/agents/*.toml` | Subagents | 從 `src/agents/*.md` 編譯出的 Codex subagents |
| `.codex/rules/*.rules` | Exec policy | 從 `src/codex/rules/` 編譯出的 Codex 命令批准規則（可選） |
| `.agents/skills/*/SKILL.md` | Skills | 從 `src/skills/*` 編譯出的 Codex skills |

Codex 會直接使用 `AGENTS.md` 作為共享指令層，所以 `src/rules/` 這類行為規則不會自動轉成 Codex `.rules`。
...
node tools/harness-compiler.js compile --source --target=codex
```

### 3. 在 IDE 中開啟

在 Cursor、Kiro、或 Codex 中開啟專案。Codex 的 project config 會在專案被標記為 trusted 後生效。
...
```bash
# 從 src/ 目錄編譯（推薦）
node tools/harness-compiler.js compile --source [--target=cursor|kiro|codex|all]
...
| 源碼 | Cursor 輸出 | Kiro 輸出 | Codex 輸出 |
|------|-------------|-----------|--------------|
| `src/rules/*.md` | `.cursor/rules/*.mdc` | `.kiro/steering/*.md` | 由 `AGENTS.md` 承接，不自動編譯 |
| `src/agents/*.md` | `.cursor/agents/*.md` | `.kiro/agents/*.md` | `.codex/agents/*.toml` |
| `src/skills/*/SKILL.md` | `.cursor/skills/*/SKILL.md` | `.kiro/skills/*/SKILL.md` | `.agents/skills/*/SKILL.md` |
| `src/hooks/*.yaml` | `.cursor/hooks/*.sh` + `hooks.json` | `.kiro/hooks/*.json` | 僅支援 Codex 專用來源（`src/codex/hooks.json`） |
| `src/codex/config.toml` | — | — | `.codex/config.toml` |
| `src/codex/rules/*.rules` | — | — | `.codex/rules/*.rules` |
