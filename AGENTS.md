...
- **Source-based workflow**: Edit `src/` → compile with `node tools/harness-compiler.js compile --source` → generates `.cursor/` and `.kiro/`
- Directory structure:
...
- **Source files**: Always edit `src/` directory, never edit compiled `.cursor/` or `.kiro/` directories.
